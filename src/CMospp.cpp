...
{
	class CMospp *m = (class CMospp *)userdata;
...
CMospp::CMospp(const char *id, bool clean_session)
{
...
int CMospp::reinitialise(const char *id, bool clean_session)
{
    int rc;
...
int CMospp::connect(const char *host, int port, int keepalive)
{
	return mosquitto_connect(m_mosq, host, port, keepalive);
}

int CMospp::connect(const char *host, int port, int keepalive, const char *bind_address)
{
	return mosquitto_connect_bind(m_mosq, host, port, keepalive, bind_address);
}

int CMospp::connect_async(const char *host, int port, int keepalive)
{
	return mosquitto_connect_async(m_mosq, host, port, keepalive);
}

int CMospp::connect_async(const char *host, int port, int keepalive, const char *bind_address)
{
	return mosquitto_connect_bind_async(m_mosq, host, port, keepalive, bind_address);
}

int CMospp::reconnect()
{
	return mosquitto_reconnect(m_mosq);
}

int CMospp::reconnect_async()
{
	return mosquitto_reconnect_async(m_mosq);
}

int CMospp::disconnect()
{
...
int CMospp::will_set(const char *topic, int payloadlen, const void *payload, int qos, bool retain)
{
	return mosquitto_will_set(m_mosq, topic, payloadlen, payload, qos, retain);
}

int CMospp::will_clear()
{
	return mosquitto_will_clear(m_mosq);
}

int CMospp::username_pw_set(const char *username, const char *password)
{
	return mosquitto_username_pw_set(m_mosq, username, password);
}

int CMospp::publish(int *mid, const char *topic, int payloadlen, const void *payload, int qos, bool retain)
{
	return mosquitto_publish(m_mosq, mid, topic, payloadlen, payload, qos, retain);
}

void CMospp::reconnect_delay_set(unsigned int reconnect_delay, unsigned int reconnect_delay_max, bool reconnect_exponential_backoff)
{
	mosquitto_reconnect_delay_set(m_mosq, reconnect_delay, reconnect_delay_max, reconnect_exponential_backoff);
}

int CMospp::max_inflight_messages_set(unsigned int max_inflight_messages)
{
	return mosquitto_max_inflight_messages_set(m_mosq, max_inflight_messages);
}

void CMospp::message_retry_set(unsigned int message_retry)
{
	mosquitto_message_retry_set(m_mosq, message_retry);
}

int CMospp::subscribe(int *mid, const char *sub, int qos)
{
	return mosquitto_subscribe(m_mosq, mid, sub, qos);
}

int CMospp::unsubscribe(int *mid, const char *sub)
{
	return mosquitto_unsubscribe(m_mosq, mid, sub);
}

int CMospp::loop(int timeout, int max_packets)
{
	return mosquitto_loop(m_mosq, timeout, max_packets);
}

int CMospp::loop_misc()
{
	return mosquitto_loop_misc(m_mosq);
}

int CMospp::loop_read(int max_packets)
{
	return mosquitto_loop_read(m_mosq, max_packets);
}

int CMospp::loop_write(int max_packets)
{
	return mosquitto_loop_write(m_mosq, max_packets);
}

int CMospp::loop_forever(int timeout, int max_packets)
{
	return mosquitto_loop_forever(m_mosq, timeout, max_packets);
}

int CMospp::loop_start()
{
	return mosquitto_loop_start(m_mosq);
}

int CMospp::loop_stop(bool force)
{
	return mosquitto_loop_stop(m_mosq, force);
}

bool CMospp::want_write()
{
	return mosquitto_want_write(m_mosq);
}

int CMospp::opts_set(enum mosq_opt_t option, void *value)
{
	return mosquitto_opts_set(m_mosq, option, value);
}

int CMospp::threaded_set(bool threaded)
{
	return mosquitto_threaded_set(m_mosq, threaded);
}

void CMospp::user_data_set(void *userdata)
{
	mosquitto_user_data_set(m_mosq, userdata);
}

int CMospp::socks5_set(const char *host, int port, const char *username, const char *password)
{
	return mosquitto_socks5_set(m_mosq, host, port, username, password);
}


int CMospp::tls_set(const char *cafile, const char *capath, const char *certfile, const char *keyfile, int (*pw_callback)(char *buf, int size, int rwflag, void *userdata))
{
	return mosquitto_tls_set(m_mosq, cafile, capath, certfile, keyfile, pw_callback);
}

int CMospp::tls_opts_set(int cert_reqs, const char *tls_version, const char *ciphers)
{
	return mosquitto_tls_opts_set(m_mosq, cert_reqs, tls_version, ciphers);
}

int CMospp::tls_insecure_set(bool value)
{
	return mosquitto_tls_insecure_set(m_mosq, value);
}

int CMospp::tls_psk_set(const char *psk, const char *identity, const char *ciphers)
{
